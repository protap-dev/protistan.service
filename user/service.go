...
func (s *Service) fetchInternalWithTx(
	...
	// Cache repo instances to avoid repeated allocations
	...
	var u *User
	if needUser {
		// internal consumers typically only need these columns
		var row User
		if err := tx.
			Table("users").
		...
		u = &row
		if req.IncludeUser {
			// Convert to InternalUserDTO to include active_role in JSON
			out.User = &InternalUserDTO{
				ID:              u.ID,
				Email:           u.Email,
	...
	// 2) Profile (public or full)
	var p *UserProfile
	if needProfile {
		if req.IncludePublicProfile && !req.IncludeProfile && !req.IncludeOnboarding {
			// only need 3 columns: avoid SELECT *
			var pub PublicUserProfile
			if err := tx.
				Table("user_profiles").
				Select("first_name, last_name, avatar_url").
				Where("user_id = ?", userID).
				Take(&pub).Error; err != nil {
				if errors.Is(err, gorm.ErrRecordNotFound) {
					if req.EnsureProfile {
						// create empty profile, then return empty public profile
						np := &UserProfile{UserID: userID}
			...
			// full profile needed (for include_profile or onboarding computation)
			prof, err := profileRepo.GetByUserID(ctx, userID)
			if err != nil {
				if errors.Is(err, ErrProfileNotFound) && req.EnsureProfile {
	...
	if needSettings {
		...
		if err != nil {
			if errors.Is(err, ErrSettingsNotFound) && req.EnsureSettings {
				settings = &UserSettings{
					UserID:             userID,
	...
	// 4) Onboarding (requires user + profile)
	if req.IncludeOnboarding {
		if u == nil {
			return errs.B().Msg("include_onboarding requires include_user").Err()
		}
		if p == nil {
			// if onboarding requested and profile missing, either ensure_profile or fail
			if req.EnsureProfile {
