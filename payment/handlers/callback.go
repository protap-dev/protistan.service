...
<body>
  <h1>Return to Protisan</h1>
	  ...
	  <script id="protisan-payment-return" type="application/json">%s</script>
	  <script>
	    (function () {
	      var payloadEl = document.getElementById("protisan-payment-return");
	      if (payloadEl) {
	        try {
	          window.__PROTISAN_PAYMENT_RETURN__ = JSON.parse(payloadEl.textContent || "{}");
	        } catch (_) {}
	      }
	      var appURL = %q;
	      if (!appURL) return;
