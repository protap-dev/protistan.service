...
func (p *NombaProvider) doRequest(ctx context.Context, method, baseURL, endpoint string, reqBody any, headers map[string]string, resBody any) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, maxNombaErrorBodyBytes))
		return fmt.Errorf("nomba %s %s status %d: %s", method, requestURL, resp.StatusCode, string(body))
	}
...
func (p *NombaProvider) issueToken(ctx context.Context) (string, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, maxNombaErrorBodyBytes))
		return "", fmt.Errorf("nomba POST %s status %d: %s", p.authURL(), resp.StatusCode, string(body))
	}
...
func (p *NombaProvider) Initialize(ctx context.Context, req *domain.InitializationRequest) (*domain.InitializationResponse, error) {
	...
	if req.AmountCents <= 0 {
		return nil, fmt.Errorf("nomba checkout amount cents must be positive")
	}
	apiReq.Order.Amount = json.Number(formatAmountCents(req.AmountCents))
...
func (p *NombaProvider) Verify(ctx context.Context, reference string) (*domain.VerificationResponse, error) {
	...
	amountCents, err := parseAmountCents(res.Data.Amount.String())
	if err != nil {
...
func formatAmountCents(amountCents int64) string {
	whole := amountCents / 100
	fractional := amountCents % 100
	if fractional == 0 {
		return strconv.FormatInt(whole, 10)
	}
	return fmt.Sprintf("%d.%02d", whole, fractional)
}
...
func parseAmountCents(raw string) (int64, error) {
	...
	if strings.ContainsAny(raw, "eE") {
		return 0, fmt.Errorf("amount exponent notation is unsupported")
	}
	...
	cents, err := strconv.ParseInt(whole+fractional, 10, 64)
	if err != nil {
		return 0, err
	}
	if cents <= 0 {
