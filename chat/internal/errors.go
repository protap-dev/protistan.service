...
// Unified error definitions for the chat service
var (
	...
	ErrAlreadyExists     = errs.B().Code(errs.AlreadyExists).Msg("resource already exists").Err()
)
...
func HandleRepositoryError(err error) error {
	...
	if errors.Is(err, domain.ErrThreadNotFound) || errors.Is(err, domain.ErrMessageNotFound) {
		return ErrNotFound
	}

	if errors.Is(err, domain.ErrDuplicateMessage) || errors.Is(err, domain.ErrThreadAlreadyExists) {
		return ErrAlreadyExists
	}

	var chatErr *domain.ChatError
	if errors.As(err, &chatErr) {
		...
		case "thread_not_found", "message_not_found":
			return ErrNotFound
		default:
			return errs.B().Code(errs.FailedPrecondition).Msg(chatErr.Message).Err()
		}
	}
