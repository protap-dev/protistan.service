...
func (r *ratesRepository) GetAllServicesGroupedByCategory(ctx context.Context, categoryID ...string) ([]domain.ServiceCategoryWithServices, error) {
	...
	query := r.db.WithContext(ctx).
		...
		`).
		Joins("LEFT JOIN services s ON sc.id = s.category_id")

	// Apply category filter if provided
	if len(categoryID) > 0 && categoryID[0] != "" {
		query = query.Where("sc.id = ?", categoryID[0])
	}
